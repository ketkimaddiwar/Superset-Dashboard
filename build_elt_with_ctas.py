...
def run_ctas(table, select_sql, primary_key=None):
    ...
    try:
        ...
        sql = f"select userid, sessionid, channel, ts, count(1) as cnt from {table} group by 1,2,3,4 order by 5 desc limit 1"
        print(sql)
        cur.execute(sql)
        result = cur.fetchone()
        ...
        if int(result[4]) > 1:
            print("!!!!!!!!!!!!")
            raise Exception(f"Duplicate rows found: {result}")
